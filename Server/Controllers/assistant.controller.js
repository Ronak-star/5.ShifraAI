...
export const askAssistant = async (req, res) => {
    try {
        ...
        if (user.enableNavigation) {
            ...
            // 🔥 FIX 1: Expanded navigation trigger words/phrases
            // "startsWith" was too strict — "switch back to homepage" never matched.
            // Now we check if ANY of these phrases appear ANYWHERE in the message.
            const navigationWords = [
                "open",
                "go",
                "go back",
            ...
            // Check navigation intent — includes instead of startsWith
            const wantsNavigation =
                navigationWords.some((word) =>
            ...
            if (wantsNavigation) {
                ...
                // 🔥 FIX 2: Special-case "home" / "homepage" / "main page"
                // since user.pages might not always contain a keyword-mapped home entry.
                const homeKeywords = [
                    "home",
                    "homepage",
                    "home page",
                ...
                const wantsHome = homeKeywords.some((word) =>
                    cleanMessage.includes(word)
                );
                ...
                if (wantsHome) {
                    ...
                    if (req.body.currentPath === homePath) {
                        return res.json({
                            success: true,
                            response: "Homepage already open",
                        });
                    }
                    ...
                    return res.json({
                        ...
                        action: "navigate",
                        path: homePath,
                        aiResponse: "Returning to the homepage now.",
                    });
                }

                // Find matching page from user's configured pages
                const matchedPage =
                    user.pages.find((page) =>
                ...
                if (matchedPage) {
                    ...
                    return res.json({
                        ...
                        // 🔥 FIX 3: send aiResponse too (frontend reads aiResponse, not response)
                        response:
                            `Opening ${matchedPage.name}`,
...
`;
    ...
    if(user.plan === "free"){
        user.totalMessages += 1
        // 🔥 FIX: don't await save — respond immediately, save in background
        user.save().catch((err) => console.log("Save error:", err))
    }
